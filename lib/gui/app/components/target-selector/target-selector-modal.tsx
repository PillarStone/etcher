...
const TargetsTable = styled(({ refFn, ...props }) => {
	return (
		<div>
			<BaseTable<DrivelistTarget> ref={refFn} {...props}></BaseTable>
		</div>
	);
})`
	> div {
...
> {
	...
	private getDisplayedTargets(enrichedDrivesData: any[]) {
		return enrichedDrivesData.filter((drive) => {
			const showIfSystemDrive = this.state.showSystemDrives
				? true
				: !drive.isSystem;
				...
				<Flex width="100%" height="100%">
					{!hasAvailableDrives() ? (
						...
						>
							...
							></TargetsTable>
							{!showSystemDrives &&
								numberOfSystemDrives - numberOfDisplayedSystemDrives > 0 && (
									<Link
										mt={15}
										mb={15}
